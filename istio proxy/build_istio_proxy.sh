...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/IstioProxy/1.2.0/build_istio_proxy.sh
...
function prepare() {
	...
	if [[ "$FORCE" == "true" ]]; then
		...
		printf -- '1:Bazel\t\tVersion: 0.25.0\n'
		printf -- '2:Envoy\n'
		printf -- '3:BoringSSL\n'
		printf -- '4:GCC\t\tVersion: gcc-7.3.0 \n'
		printf -- '5:Go\t\tVersion: go1.12.5\n\n'
...
function cleanup() {
	...
	rm -rf "${CURDIR}/go1.12.5.linux-s390x.tar.gz"
...
function buildGO() {
	cd "${CURDIR}"
	if command -p "go" version | grep 1.12.5 >/dev/null; then
		printf -- "Go detected\n"
	else
...
function installDependency() {
	...
	if command "bazel" version | grep 0.25.0 >/dev/null; then
...
"ubuntu-16.04")
	...
	printf -- '\nInstalling dependencies \n' |& tee -a "$LOG_FILE"
	sudo apt-get update
	sudo apt-get install -y git tar pkg-config zip g++ zlib1g-dev unzip python libtool automake cmake curl wget build-essential realpath ninja-build clang-format-5.0 
	buildGCC
	installDependency
...
"ubuntu-19.04")
	...
	printf -- '\nInstalling dependencies \n' |& tee -a "$LOG_FILE"
	sudo apt-get update
	sudo apt-get install -y git pkg-config zip zlib1g-dev unzip python libtool automake cmake curl wget build-essential rsync clang gcc-7 g++-7 libgtk2.0-0 ninja-build clang-format-8
	sudo rm -rf /usr/bin/gcc /usr/bin/g++ /usr/bin/cc
	sudo ln -sf /usr/bin/gcc-7 /usr/bin/gcc
	sudo ln -sf /usr/bin/g++-7 /usr/bin/g++
	sudo ln -sf /usr/bin/gcc /usr/bin/cc
